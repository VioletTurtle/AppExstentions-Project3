...
public class HotbarSlot : ItemSlotUI, IDropHandler
{
    ...
    public void UseSlot(GameObject Player)
    {
        ...
        if (SlotItem != null)
        {
            ...
            {
                if (SlotItem is ConsumableItem item)
                {
                    if (HS.Health == 100) { return; }
                }
                if (SlotItem is ArmorItem armoIitem)
                {
                    if (HS.armor == 5) { return; }
                }
                Debug.Log($"Used: {SlotItem.Name}");
                SlotItem.Use(Player);
                inventory.UpdateQuantity(inventoryItem);
                UpdateSlotUI();
